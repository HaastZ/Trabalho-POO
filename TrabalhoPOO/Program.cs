...
// Cadastro Funcionario (na aplicação, como um usuário)
...
{
    Console.WriteLine($"Passageiro: {passagensPassageiro.GetPassageiro().getNome()}, Passagens:");
    foreach (var vooPassagem in passagensPassageiro.getVoos())
    {
        Console.WriteLine($" - {vooPassagem.getAeroportoOrigem().getNome()} para {vooPassagem.getAeroportoDestino().getNome()}, pela: {vooPassagem.getCompanhiaAerea().getNome()}");
    }
}
